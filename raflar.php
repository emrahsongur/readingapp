...
// Tüm rafları ve her rafın kitaplarını çek
try {
    $stmtRaflar = $pdo->prepare("
        SELECT id, etiket, aciklama
        FROM raflar
        WHERE user_id = :user_id
        ORDER BY etiket ASC
    ");
    $stmtRaflar->execute(['user_id' => $user_id]);
    $raflar = $stmtRaflar->fetchAll(PDO::FETCH_ASSOC);

    foreach ($raflar as &$raf) {
        $stmtKitaplar = $pdo->prepare("
            SELECT k.id, k.baslik, k.yazar, k.kapak, k.durum_id, k.sayfa, k.baslangic_sayfa, k.bitis_sayfa,
                   k.kitap_tipi_id, k.sesli_toplam_saniye,
                   d.durum as durum_adi, kt.ad as kitap_tipi_adi,
                   o.ilk_baslama, o.son_sayfa, o.toplam_saniye, o.son_bitis, o.son_yuzde, o.son_sure_saniye,
                   COALESCE(al.alinti_sayisi, 0) as alinti_sayisi,
                   COALESCE(du.dusunce_sayisi, 0) as dusunce_sayisi
            FROM kitaplar k
            INNER JOIN kitap_raf kr ON k.id = kr.kitap_id
            ...
            WHERE kr.raf_id = :raf_id AND k.user_id = :user_id
            ORDER BY k.baslik ASC
        ");
        $stmtKitaplar->execute(['raf_id' => $raf['id'], 'user_id' => $user_id]);
        $raf['kitaplar'] = $stmtKitaplar->fetchAll(PDO::FETCH_ASSOC);
    }
    unset($raf);
...
<head>
    ...
    <title>Raflar - Reading App</title>
    <style>
        body {
        ...
        .empty-shelf { color: #9ca3af; font-size: 0.9rem; font-style: italic; }
        .empty-state {
            text-align: center;
            padding: 3rem;
            color: #6b7280;
    ...
    <?php if (count($raflar) > 0): ?>
        <?php foreach ($raflar as $raf): ?>
            <section class="shelf-section">
                <h3 class="shelf-title"><?= htmlspecialchars($raf['etiket']) ?></h3>
                <?php if (!empty($raf['aciklama'])): ?>
                    <p class="shelf-aciklama"><?= htmlspecialchars($raf['aciklama']) ?></p>
                <?php endif; ?>

                <?php if (count($raf['kitaplar']) > 0): ?>
                    <div class="shelf-books">
                        <?php foreach ($raf['kitaplar'] as $kitap): ?>
                            <?php
                            $tip_id = (int)($kitap['kitap_tipi_id'] ?? 1);
                            $sayfa = (int) ($kitap['sayfa'] ?? 0);
                            ...
                            <div class="book-card">
                                ...
                                <div class="book-meta">
                                    ...
                                    <?php if ($tip_id === 1): ?>
                                        <div><?= $sayfa ?> sayfa</div>
                                        <?php if ((int)($kitap['durum_id'] ?? 0) === 2 && $bitis_eff > 0): ?>
                                    ...
                                    <?php if ($has_seans): ?>
                                        ...
                                        <?php endif; ?>
                                        <?php if ((int)($kitap['durum_id'] ?? 0) === 3 && !empty($kitap['son_bitis'])): ?>
                ...
                <?php else: ?>
                    <p class="empty-shelf">Bu rafta henüz kitap yok.</p>
                <?php endif; ?>
            </section>
        <?php endforeach; ?>
    <?php else: ?>
        <div class="empty-state">
            <h3>Henüz raf oluşturmadınız.</h3>
            <p>Üstteki <strong>+ Raf Ekle</strong> ile raf oluşturup kitaplarınızı etiketleyebilirsiniz.</p>
        </div>
    <?php endif; ?>
